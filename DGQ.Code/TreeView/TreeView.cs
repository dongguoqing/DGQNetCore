            ...
            if (item.Count > 0)
            {
                try
                {
                    foreach (TreeViewModel entity in item)
                    {
                        ...
                        if (entity.text != null && !string.IsNullOrEmpty(entity.text.Replace("&nbsp;", "")))
                        {
                        ...
                        strJson.Append("\"show-checkbox\":" + entity.showcheck.ToString().ToLower() + ",");
                        strJson.Append("\"expand\":" + entity.isexpand.ToString().ToLower() + ",");
